...
"""Build the Horizon Foundation bespoke site.
...
Usage:
    ...
    ./site/build.py serve        # build then serve on :8003 for local preview
...
def build():
    print(f"Building Horizon Foundation → {DIST_DIR}")
    ...
    pages = [
        ...
        ("programs.html.j2", "programs.html"),
    ...
    if not cname_file.exists():
        cname_file.write_text("horizonfoundation.eduserver.au\n", encoding="utf-8")
        print("  ✓ CNAME")
...
def serve():
    """Build then serve the dist directory on :8003 for local preview."""
    import http.server
    import socketserver
    ...
    with socketserver.TCPServer(("", PORT), http.server.SimpleHTTPRequestHandler) as httpd:
        print(f"\nServing Horizon at http://localhost:{PORT}")
